#!/usr/bin/env bash
# E3: Vanilla GRPO 基线 — 原始 schema, 标准 GRPO
# Config: configs/exp3_vanilla_grpo.yaml（可通过 EXP_CONFIG 覆盖）
set -euo pipefail
...
EXP_CONFIG="${EXP_CONFIG:-${PROJECT_DIR}/configs/exp3_vanilla_grpo.yaml}"
...
GROUP_SIZE="${GROUP_SIZE:-$(get rollout.group_size)}"
MAX_TURNS="${MAX_TURNS:-$(get rollout.max_turns)}"
...
# ── batch size 自适应：train_batch = lcm(mini_batch, group_size)，保证整除 group ──
...
# 如需启用 flashinfer，调用时覆盖这两个变量即可
export VLLM_USE_FLASHINFER_SAMPLER="${VLLM_USE_FLASHINFER_SAMPLER:-0}"
export VLLM_ATTENTION_BACKEND="${VLLM_ATTENTION_BACKEND:-FLASH_ATTN}"

echo "E3 Vanilla GRPO start | model=${MODEL_PATH} | gpus=${N_GPUS} | micro=${MICRO_BATCH_PER_GPU} | train_batch=${TRAIN_BATCH_SIZE} | mini_batch=${MINI_BATCH_SIZE} | group=${GROUP_SIZE} | config=${EXP_CONFIG}" \

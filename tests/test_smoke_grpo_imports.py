...
策略：装了的依赖必须落在 [min, max) 兼容窗口；没装的依赖（verl/vllm/bfcl_eval 在
非训练机上不一定有）跳过，不算失败。

兼容窗口锚定 arl 环境实测通过的版本组合（见 docs/archive/review.md）：
torch 2.8 / vllm 0.11 / verl 0.6.1 / flash_attn 2.7.3 / transformers 4.57.6 /
ray 2.x / bfcl_eval 2025.10."""
...
def _require_in_range(mod_name: str, min_v: str, max_v: str):
    ...
    assert lo <= v < hi, (
        ...
        f"如确认要升级，请同步更新 docs/archive/review.md / docs/project_status.md 的版本表，再放宽本测试。"
    )
...
def test_flashinfer_optional_or_disabled():
    ...
    assert v < Version("0.7.0"), (
        ...
        f"如确实要升级，需重新跑一遍 E3/E4/E5 smoke。"
    )
...
def test_verl_entry_symbols():
    """run_exp4.py 直接依赖 verl.trainer.main_ppo.run_ppo + TaskRunner。"""
...
def test_verl_advantage_registry_symbols():
    """run_exp4 用 register_adv_est 注入 schemashift_grpo，符号丢失会导致整个
    SchemaShift 路径失效。"""
    _require_symbols(
...
def test_bfcl_eval_importable():
    """agent_loop 直接调 bfcl_eval，运行时缺它训练就起不来。"""
    pytest.importorskip("bfcl_eval", reason="bfcl_eval 未安装，agent rollout 不可用")
    # 不强约束子符号，bfcl_eval 内部模块路径升级时会变
    # （P0-2 重构后会通过 envs adapter 隔离）
...
def test_project_entry_imports():
    """三个 GRPO 入口脚本对应的核心模块必须能 import，且不会触发副作用错误。"""
    importlib.import_module("src.training.run_exp4")
    importlib.import_module("src.training.schemashift_grpo_estimator")
    importlib.import_module("src.training.register_estimator")
    importlib.import_module("src.agent_loop.bfcl_agent_loop")
...
def test_env_matches_documented_versions(record_property):
    ...
    for name in [
        ...
        "flash_attn", "flashinfer_python", "bfcl_eval", "tensordict",
    ]:
